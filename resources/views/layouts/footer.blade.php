            ...
            for (let day = 1; day <= totalDays; day++) {
                ...
                if (hasEvents) {
                    ...
                    displayEvents.forEach(event => {
                        ...
                        const borderColor = (event.is_missed === 1 || event.is_overdue === 1) ?
                            '#dc3545' : (isLead ? '#3762b8' : '#28a745');
                        const missedClass = (event.is_missed === 1 || event.is_overdue === 1) ?
                            'missed' : '';
                    });
                }
    ...
    window.showDayEvents = function(dateStr) {
        ...
        if (events.length === 0) {
            ...
            events.forEach(event => {
                const isLead = event.event_type === 'lead';
                const icon = isLead ? 'fa-user' : 'fa-tasks';
                const borderColor = (event.is_missed === 1 || event.is_overdue === 1) ? '#dc3545' : (
                    isLead ? '#3762b8' : '#28a745');
            ...
            const dialogConfig = {
                start: {
                    ...
                    html: `<div class="swal-work-start">
                                        <i class="fas fa-business-time fa-3x mb-3" style="color:#3762b8"></i>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function updateFormProgress() {
            ...
            $('#formProgressText').text(progressPercentage === 100 ? 'Ready to submit!' :
            ...
            tabs.forEach(tab => {
                ...
                const tabPercentage = tabFields.length > 0 ? Math.round((tabCompleted / tabFields
                    .length) * 100) : 100;
    ...
    function applyWebsiteStyles(styles) {
        ...
        if (styles.font_primary) {
            ...
            primaryFont.href =
                `https://fonts.googleapis.com/css2?family=${encodeURIComponent(styles.font_primary)}:wght@300;400;500;600;700&display=swap`;
        ...
        if (styles.font_secondary) {
            ...
            secondaryFont.href =
                `https://fonts.googleapis.com/css2?family=${encodeURIComponent(styles.font_secondary)}:wght@400;500;600;700&display=swap`;
                ...
                $(this).find('td').each(function(index) {
                    if ((index > 0 || !$(this).find('input[type="checkbox"]').length)) {
                        ...
                        if ($(this).find('.comment-text').length) {
                            ...
                            let text = $(this).find('h6').text().trim() + ' - ' + $(
                                this).find('.text-muted').text().trim();
                            text = text.replace(/View more/ig, '').trim();
                            td.textContent = text;
                ...
                $(this).find('td').each(function(index) {
                    if ((index > 0 || !$(this).find('input[type="checkbox"]').length)) {
                        ...
                        if ($(this).find('.comment-text').length) {
                            ...
                            text = $(this).find('h6').text().trim() + ' - ' + $(this)
                                .find('.text-muted').text().trim();
                        } else if ($(this).find('.cust-badge').length) {
        ...
        $('#modalTeamSelect').on('change', function() {
            ...
            if (teamId) {
                ...
                $('#pointsContainer').html(
                    '<div class="alert alert-warning">Please select a team first</div>');
            }
        });
        ...
        function initializeTargetRows() {
            ...
            container.addEventListener('click', function(e) {
                ...
                if (e.target.closest('.add-row')) {
                    ...
                    clone.querySelectorAll('input[name="week[]"]').forEach(i => i.value = row
...
<script>
    $(document).ready(function() {
        ...
        $('#exhibitionForm').submit(function(e) {
            ...
            saveBtn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-1"></i> ' + (
                    ...
                    if (result.value.success) {
                        Swal.fire({
                            ...
                            html: result.value.message +
                                `<br>Converted: ${result.value.converted_count}<br>Failed: ${result.value.failed_count}`,
                            icon: 'success',
                            confirmButtonText: 'OK'
                        }).then(() => {
                            window.location.reload();
