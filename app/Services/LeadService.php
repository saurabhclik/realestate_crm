...
class LeadService
{
    ...
    public function updateLead($request, $id, $isMobile = false)
    {
        ...
        try {
            ...
            if ($allocationChanged) {
                ...
                $statusText = in_array($newUserRole, ['admin', 'team_manager']) ? 'allocated_lead' : 'NEW LEAD';
    ...
    public function quickLead($request)
    {
        ...
        try {
            ...
            $status = in_array($userType, ['admin', 'team_manager']) ? 'allocated_lead' : 'NEW LEAD';
            $isAllocated = in_array($userType, ['admin', 'team_manager']) ? 0 : 1;
