...
@section('content')
    ...
    @php
        ...
        foreach ($leadStatuses as $status) {
            ...
            $routeName = !empty($status->route_name) ? $status->route_name : null;
            ...
            $value = isset($leadStatsArray[$propertyName]) ? $leadStatsArray[$propertyName] : 0;
        ...
        $leadCards[] = [
            'route' => null,
            'title' => 'Other Leads',
            'value' => max(($leadStatsArray['total_lead'] ?? 0) - $mainLeadTotal, 0),
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0 font-size-18">Dashboard<div
                                class="border-bottom border-3 border-primary mb-2 mt-1 w-75"></div>
                        </h4>
                        <div class="d-flex align-items-center flex-wrap gap-2">
                            ...
                            <a href="{{ route('data-center.create') }}" class="btn btn-primary btn-sm circle-btn"
                                title="Data Center">
                                <i class="fas fa-database"></i>
                            </a>

                            <button type="button" class="btn btn-info btn-sm circle-btn" data-bs-toggle="modal"
                                data-bs-target="#importModal" title="Bulk Import">
                                <i class="fas fa-file-import"></i>
                            </button>
                            <a class="btn btn-warning btn-sm circle-btn" href="{{ route('lead.add') }}"
                                title="Add Manual Lead">
                                <i class="fas fa-plus"></i>
                            </a>
                            <button class="btn btn-success btn-sm circle-btn" data-bs-toggle="modal"
                                data-bs-target="#quickLeadModal" title="Quick Add Lead">
                                <i class="fas fa-bolt"></i>
                            </button>
                            <a href="{{ route('settings.ratings') }}" class="btn btn-outline-warning btn-sm circle-btn"
                                title="View Ratings">
                                <i class="fas fa-star"></i>
                            </a>
                            <input type="text" class="form-control form-control-sm date-range-filter"
                            ...
                            <select class="form-control form-control-sm" id="agent-filter" style="width: 150px;"
                                title="Select Agent">
                        ...
                        <div class="card-body pt-0 pb-3">
                            <div class="row">
                                <div class="col-sm-4">
                                    ...
                                    <p class="text-muted mb-0 text-truncate">{{ session('user_type', '') }}</p>
                                </div>
                                <div class="col-sm-8">
                                    <div class="pt-2">
                                        <div class="row">
                                            <div class="row">
                                                <div class="col-6">
                                                    ...
                                                    @if ($userType == 'admin' || ($isSpecial == 1 && in_array('lead.all_lead', $masterAccess)))
                                                        <a href="{{ route('lead.all_lead', ['user' => $selectedAgent]) }}"
                            ...
                            <div class="row g-2">
                                @foreach ($othersLeadData as $other)
                                    @php
                                        $canAccess =
                                            $userType == 'admin' ||
                                            ($isSpecial == 1 && in_array($other['route'], $masterAccess));
                                    @endphp
                                            ...
                                            <div class="card-body py-2 px-3">
                                                <div class="d-flex align-items-center justify-content-between">
                                                    <div class="flex-grow-1">
                                                        ...
                                                        @endif
                                                        <p class="text-muted mb-1 small fw-medium">
                                                            {{ $other['title'] ?? 0 }}</p>
                                                        <h6 class="mb-0 fw-semibold">{{ $other['value'] ?? 0 }}</h6>
                                                    </div>
                                                    <div class="flex-shrink-0 ms-3">
                                                        <div
                                                            class="avatar-xs rounded-circle bg-primary text-white d-flex align-items-center justify-content-center">
                                                            <i
                                                                class="bx {{ $other['icon'] ?? 'bx-help-circle' }} font-size-16"></i>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
