...
@section('content')
                ...
                <div class="col-12">
                    <div class="page-title-box d-flex align-items-center justify-content-between">
                        ...
                        <div class="d-flex gap-2">
                            <button id="btnExportExcel" class="shadow btn btn-success btn-sm d-flex align-items-center"
                                data-bs-toggle="tooltip" data-bs-placement="top" title="Export table data to Excel">
                                <i class="fas fa-file-excel me-2"></i> Excel
                            </button>

                            <button id="btnExportPDF" class="shadow btn btn-danger btn-sm d-flex align-items-center"
                                data-bs-toggle="tooltip" data-bs-placement="top" title="Export table data to PDF">
                ...
                <div class="col-12">
                    <div class="card p-3">
                        @if(in_array($lead_name, ['allocate']))
                            <form class="lead-allocate-form"
                                action="{{ $lead_name === 'allocate' ? route('lead.allocate_user') : route('lead.transfer_user') }}"
                                method="POST">
                                @csrf
                                <div class="row mb-3">
                                    <div class="col-md-4">
                                        <label for="user">Select User to
                                            {{ $lead_name === 'allocate' ? 'Allocate' : 'Transfer' }}</label>
                                    ...
                                    <div class="col-md-3 d-flex align-items-center">
                                        <div class="form-check mt-4">
                                            <input type="checkbox" class="form-check-input" name="send_to_new_lead"
                                                id="sendToNewLead" value="1">
                                            <label class="form-check-label" for="sendToNewLead">
                                                <strong>Send to New Lead</strong>
                                                <small class="d-block text-muted">If checked, lead goes to NEW LEAD
                                                    status</small>
                                            </label>
                                        </div>
                                    </div>
                        ...
                        @endif
                            <div>
                                ...
                                <span id="local-loader"
                                    class="d-none d-inline-flex align-items-center ms-2 fw-bold text-primary">
                                    <div class="spinner-border spinner-border-sm me-2" role="status"></div>
                                    <span id="local-timer">0s</span>
                                </span>
                            </div>
                                ...
                                <tbody>
                                    @foreach($leads as $row)
                                        ...
                                        <tr>
                                            @if($lead_name == 'allocate' || $lead_name == 'transfer')
                                                <td>
                                                    <input type="checkbox" class="form-check-input checked" name="checked[]"
                                                        value="{{ $row->id }}">
                                                </td>
                                            @endif
                                            {{-- <td>
                                                ...
                                                <div class="d-flex gap-3 align-items-center">
                                                    <div class="position-relative d-inline-block text-center">
                                                        ...
                                                        @if(session('user_type') == 'admin' || session('user_type') == 'team_manager')
                                                            <div class="action-item duplicate-item"
                                                                onclick="showDuplicateModal('{{ $row->id }}')"
                                                                data-bs-toggle="tooltip" title="Duplicate Lead"
                                                    ...
                                                    @if(session('user_type') == 'admin' || session('user_type') == 'team_manager')
                                                        <div class="action-item share-item"
                                                            onclick="showShareModal('{{ $row->id }}', '{{ $row->lead_shared_with }}')"
                                                            data-bs-toggle="tooltip" title="Share Lead" style="cursor:pointer;">
                                            ...
                                            <td>
                                                <div class="d-flex flex-column">
                                                    ...
                                                    <div class="d-flex justify-content-between align-items-center">
                                                        ...
                                                        <div class="d-flex">
                                                            <a href="tel:{{ $phone }}" class="btn btn-xs btn-soft-light"
                                                            ...
                                                            @if($lead_name === 'completed')
                                                                <button class="btn btn-xs btn-soft-light view-applicant-btn"
                                                                    ...
                                                                    data-id="{{ $row->id }}" data-app-city="{{ $row->app_city }}"
                                                            ...
                                                            @if(session('user_type') == 'admin')
                                                                <button class="btn btn-xs btn-soft-light delete-lead-btn"
                                                                    data-lead-id="{{ $row->id }}" data-lead-name="{{ $row->name }}"
                                                                    data-bs-toggle="tooltip" title="Delete Lead">
                                                                    <i class="fas fa-trash text-danger"></i>
                                                                </button>
                                                            @endif
                                                            @if($softwareType === 'real_state')
                                                                <button class="btn btn-xs btn-soft-light matching-projects-btn"
                                                                    data-lead-id="{{ $row->id }}" data-lead-phone="{{ $phone }}"
                                                            ...
                                                            @if($row->conversion_type === 'Completed')
                                                                <button class="btn btn-xs btn-soft-light view-post-sale-btn"
                                                                    data-lead-id="{{ $row->id }}" data-bs-toggle="modal"
                                                        ...
                                                        @if(count($sharedUsers) > 0)
                                                            <div class="d-flex flex-wrap gap-1">
                                                                @foreach($sharedUsers as $sharedUser)
                                                                    <span
                                                                        class="badge bg-success me-1 d-inline-flex align-items-center gap-1">
                                                                        {{ $sharedUser->name }}
                                                                        <button type="button"
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        ...
                                                        <div class="flex-grow-1">
                                                            ...
                                                            <a href="javascript:void(0);" onclick="showComment('{{ $row->id }}')"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="applicant-section">
                        <div class="applicant-section-title">{{ $isLeadManagement ? 'Product Details' : 'Project Details' }}
                        </div>
                        <div class="applicant-detail">
                ...
                <form id="deleteLeadForm" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="alert alert-warning">
                            ...
                            <strong>Warning:</strong> This action cannot be undone. All lead data, comments, and history
    ...
    <div class="modal fade" id="whatsappShareModal" tabindex="-1" aria-labelledby="whatsappShareModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            <div class="form-check">
                                <input class="form-check-input whatsapp-section" type="checkbox" value="image"
                                    id="shareSectionImage" checked>
                                <label class="form-check-label" for="shareSectionImage">Image</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check">
                                <input class="form-check-input whatsapp-section" type="checkbox" value="highlights"
                                    id="shareSectionHighlights" checked>
                                <label class="form-check-label" for="shareSectionHighlights">Highlights</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check">
                                <input class="form-check-input whatsapp-section" type="checkbox" value="location"
                                    id="shareSectionLocation" checked>
                                <label class="form-check-label" for="shareSectionLocation">Location</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check">
                                <input class="form-check-input whatsapp-section" type="checkbox" value="pricing"
                                    id="shareSectionPricing" checked>
                                <label class="form-check-label" for="shareSectionPricing">Pricing & Status</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check">
                                <input class="form-check-input whatsapp-section" type="checkbox" value="amenities"
                                    id="shareSectionAmenities" checked>
                                <label class="form-check-label" for="shareSectionAmenities">Amenities</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check">
                                <input class="form-check-input whatsapp-section" type="checkbox" value="contact"
                                    id="shareSectionContact" checked>
                                <label class="form-check-label" for="shareSectionContact">Admin Contact</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check">
                                <input class="form-check-input whatsapp-section" type="checkbox" value="cta"
                                    id="shareSectionCta" checked>
    ...
    <script>
        ...
        // Create a variable to remember the timer ID
        var myTimer;
        // Force pagination links to keep the 'length' parameter
        $(document).on('click', '.pagination a', function (e) {
            e.preventDefault(); // Stop the link from opening immediately

            // Get the URL of the page you clicked (e.g., page 2)
            var url = new URL(this.href);

            // Get the current value from your dropdown (e.g., 25)
            var currentLength = $('#lengthSelect').val();

            // Add/Update the length parameter in the URL
            url.searchParams.set('length', currentLength);

            // Go to the corrected URL
            window.location.href = url.toString();
        });
        $('#lengthSelect').on('change', function () {
            // 1. If a timer is already running, stop it immediately (prevents the blink/mess up)
            if (myTimer) {
                clearInterval(myTimer);
            }

            // 2. Reset the timer text to 0.0s
            $('#local-timer').text('0.0s');

            // 3. Make sure the loader is visible
            $('#local-loader').removeClass('d-none').fadeIn(200);

            // 4. Start the new timer and save it to 'myTimer'
            var s = Date.now();
            myTimer = setInterval(function () {
                $('#local-timer').text(((Date.now() - s) / 1000).toFixed(1) + 's');
            }, 100);
